...
# 새로운 비번으로 변경하는 메소드
def updatePassWord(uIDX, newPW) :
    if not uIDX or not newPW :
        return False
    dbm.DBOpen(host, user, passwd, dbname)
    # 비밀번호 보안 적용 예시 SQL문
    #sql = ' select * from UserList where uIDX = %s and uPW = md5(%s) '
    #result = validateUser(uIDX, newPW)
    sql = ' select * from UserList where uIDX = %s and uPW = %s '
    datas = (uIDX, newPW)
    dbm.OpenSQL(sql, datas )
    result = dbm.getData(0)
    dbm.CloseSQL()
    if result :
        # 새 비번을 보안적용한 내용이 기존 비번과 같음
        return 'duplicate'
    # 새 비번은 새로운 비번이 맞음
    sql = ' update UserList set uPW = %s where uIDX = %s '
    datas = (newPW, uIDX)
    result = dbm.RunSQL(sql, datas)
    dbm.DBClose()
    return result
