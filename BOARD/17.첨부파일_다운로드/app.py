...
# <img src='/image/이미지파일명'> 에 대응하는 라우트 함수
@app.route('/image/<string:filename>')
def image_route(filename):
    # 원본 파일명이 크게 상관 없음
    # filename : UUID를 사용한 저장파일명
    full_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
    if not os.path.exists(full_path) :
        return 'FILE Not Found', 404
    return send_from_directory(
        app.config['UPLOAD_FOLDER'],    # 폴더
        filename                        # 파일명
    )

# <a href='/file/첨부파일번호'> 에 대응하는 라우트 함수
@app.route('/file/<int:aIDX>')
def file_route(aIDX):
    # 원본 파일명을 제공해야 함
    # aIDX로 DBMS에서 원본파일명 / 저장파일명을 조회
    info = boardService.getFileInfo(aIDX)
    return send_from_directory(
        app.config['UPLOAD_FOLDER'],        # 폴더
        info.get('stoName'),                # 파일명
        as_attachment = True,               # 첨부파일 여부
        download_name = info.get('oriName') # 다운로드시 파일명 지정
    )
